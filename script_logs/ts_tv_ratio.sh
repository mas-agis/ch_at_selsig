...
#change directory to folder containing phased vcfs
cd /home/masagis/data/third_project/aa_phasing
...
do
	for i in {1..29}
	do 
		vcftools --gzvcf phased_$i.gt.vcf.gz --keep $x --TsTv-summary --out $output/$i/$x
	done
done
