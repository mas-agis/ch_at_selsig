...
output="/home/masagis/data/third_project/safe"
...
do
  python2 $iSAFE --input $input/$1/$1_$2.recode.vcf.gz --output $output/$1/$i --AA $aa/aa_$2.fa --region "$2:${start[$i]}-${start[$i+1]}" --IgnoreGaps --MinRegionSize-ps 100 --SAFE
  ((i++))
done
...
while [[ $i -lt $length_index ]]
do 
	cat $output/$1/$i.SAFE.out | sed '1d' >> $output/$1/final_$2.txt
	((i++))
done

rm $output/$1/*.iSAFE.out
